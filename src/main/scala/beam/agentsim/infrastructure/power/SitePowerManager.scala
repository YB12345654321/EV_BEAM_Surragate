...
class SitePowerManager(chargingNetworkHelper: ChargingNetworkHelper, beamServices: BeamServices) extends LazyLogging {
  ...
  beam.agentsim.agents.pricing.DynamicPriceProvider.loadCsv(
    "test/input/smart-charging/price.csv",
    0.25
  )
  logger.info("Loaded smart charging dynamic price file: test/input/smart-charging/price.csv")
  ...
  def dispatchEnergy(
    ...
    val currentPrice =
      beam.agentsim.agents.pricing.DynamicPriceProvider.getPrice(cycleStartTime)

    val timeToDepartureSeconds =
      Math.max(0, chargingVehicle.estimatedDepartureTime - cycleStartTime)

    val isHouseholdHomeCharging =
      chargingVehicle.stall.parkingType.toString == "Residential" &&
      chargingVehicle.activityType.equalsIgnoreCase("Home")

    val capacityJ = vehicle.beamVehicleType.primaryFuelCapacityInJoule
    val currentSocJ = vehicle.primaryFuelLevelInJoules
    val userProfile =
      beam.agentsim.agents.pricing.HouseholdChargingUserProfile.profileForVehicle(vehicle.id.toString)
    val targetSocRatio = userProfile.targetSocRatio
    val referencePrice = beam.agentsim.agents.pricing.HouseholdSmartChargingManager.ReferencePrice
    val targetJ = targetSocRatio * capacityJ
    val deficitJ = math.max(0.0, targetJ - currentSocJ)
    val socRatio = if (capacityJ > 0.0) currentSocJ / capacityJ else 0.0

    val chargingDecisionContext =
      beam.agentsim.agents.pricing.ChargingDecisionContext(
        tick = cycleStartTime,
        vehicleId = vehicle.id.toString,
        household = isHouseholdHomeCharging,
        parkingType = chargingVehicle.stall.parkingType.toString,
        activityType = chargingVehicle.activityType,
        currentPrice = currentPrice,
        referencePrice = referencePrice,
        targetSocRatio = targetSocRatio,
        socJ = currentSocJ,
        capacityJ = capacityJ,
        socRatio = socRatio,
        targetJ = targetJ,
        deficitJ = deficitJ,
        timeToDepartureSeconds = timeToDepartureSeconds,
        durationSeconds = duration,
        chargerPowerKW = rawPowerLimit,
        estimatedDeparture = chargingVehicle.estimatedDepartureTime,
        userSegment = userProfile.segment,
        priceTolerance = userProfile.priceTolerance,
        urgentTimeThresholdSeconds = userProfile.urgentTimeThresholdSeconds,
        acceptedPriceThreshold = referencePrice * (1.0 + userProfile.priceTolerance)
      )

    val smartChargingDecision =
      if (isHouseholdHomeCharging) {
        beam.agentsim.agents.pricing.HouseholdSmartChargingManager.decide(chargingDecisionContext)
      } else {
        beam.agentsim.agents.pricing.ChargingDecision(
          shouldCharge = true,
          reason = "non_household_bypass",
          powerMultiplier = 1.0,
          finalPowerKW = rawPowerLimit,
    ...
    val constrainedPowerAtVehicleLevel =
      smartChargingDecision.finalPowerKW

    beam.agentsim.agents.pricing.SmartChargingCsvWriter.write(
      tick = cycleStartTime,
      ...
      activityType = chargingVehicle.activityType,
      price = currentPrice,
      decision = smartChargingDecision.reason,
      userSegment = chargingDecisionContext.userSegment,
      priceTolerance = chargingDecisionContext.priceTolerance,
      targetSocRatio = chargingDecisionContext.targetSocRatio,
      urgentTimeThresholdSeconds = chargingDecisionContext.urgentTimeThresholdSeconds,
      acceptedPriceThreshold = chargingDecisionContext.acceptedPriceThreshold,
      duration = duration,
      rawPowerKW = rawPowerLimit,
      finalPowerKW = constrainedPowerAtVehicleLevel,
      socJ = vehicle.primaryFuelLevelInJoules,
      socRatio = smartChargingDecision.socRatio,
      deficitJ = smartChargingDecision.deficitJ,
      targetJ = smartChargingDecision.targetJ,
      capacityJ = smartChargingDecision.capacityJ,
      estimatedDeparture = chargingVehicle.estimatedDepartureTime
    )
